...
const infiltrationGames = [
	...
	},
	{
		name: "enter the code",
		init: function (screen) { 
      state.game.frameDelay = 25
    },
		play: function (screen) {
      if (state.game.frameDelay > 0) {
    ...
    // TODO: Fix this one
        ...
        // pressKey(" ", 50, () => state.game.data = "done"); // TODO: this fails the task
        //setTimeout(() => state.game.data = "done", 1000); // TODO: for now, try it manually
			}

			// Attack in next frame - instant attack sometimes
			// ends in failure.
      // MAYBE NEED TO LOOK AT THIS: 'Preparing?'
      // const compareString = 'Preparing?'
      const compareString = "ATTACKING!"
			if ('wait' === state.game.data && -1 !== data.indexOf(compareString)) {
        console.log(compareString) // TODO: THIS WORKS
        pressKey(" ")
        state.game.data = "done"
				// state.game.data = "attack"; // failure every time
			}
		},
	},
	{
 ...
 */
function pressKey(keyOrCode, delay, cb) {
	let keyCode = 0;
	let key = "";
  ...
  if (delay) {
	...
	// sendEvent("keypress"); // TODO: I added this
	// sendEvent("keyup"); // TODO: I added this
}
