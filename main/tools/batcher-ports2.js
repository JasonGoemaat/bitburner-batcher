...
export async function main(ns) {
  ...
  let LOG_DELAY = 60000
  let next = Math.ceil((activeStart - start) / 60000) * 60000 + new Date().valueOf()
  let logs = []
  while (true) {
    ...
    let table = createTable(logs.map(x => ({
      ...
      active: x.activeTime ? ns.nFormat(x.activeTime / 1000, '00:00:00') : 'ERR',
