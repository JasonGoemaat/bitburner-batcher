	...
	while (true) {
		...
		div.cities.forEach(city => {
          ...
          if (!div.products.find(x => x === weightName)) {
            ...
            ns.print(`${weightName} want ${fnum(targetStorage, 0)} space or ${fnum(targetAmount, 0)} units, need ${fnum(neededAmount)}, buying ${fnum(purchaseAmountPerSecond, 0)}/s (${purchaseAmountPerSecond}):`); // + JSON.stringify(args))
